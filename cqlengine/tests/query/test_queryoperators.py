...
class TestQuerySetOperation(BaseCassEngTestCase):
    ...
    def test_maxtimeuuid_function(self):
        ...
        assert qry.cql == '"time" = MaxTimeUUID(%({})s)'.format(qry.value.identifier)
    ...
    def test_mintimeuuid_function(self):
        ...
        assert qry.cql == '"time" = MinTimeUUID(%({})s)'.format(qry.value.identifier)
    ...
    def test_token_function(self):
        ...
        self.assertEquals(q._where[0].cql, 'token("p1", "p2") > token(%({})s, %({})s)'.format(*func.identifier))
